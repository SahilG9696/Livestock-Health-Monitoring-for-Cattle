...
const DashboardPage = () => {
  ...
  const [cowsData, setCowsData] = useState<any[]>([]);
  const [filterType, setFilterType] = useState<'all' | 'healthy' | 'alerts'>('all');
  const [cowStats, setCowStats] = useState<any[]>([]);
    ...
    const unsubscribe = onValue(cowsRef, (snapshot) => {
      ...
      if (data) {
        ...
        cowsList.forEach(cowId => {
          ...
          if (cowHistory) {
            const dataArray = Object.entries(cowHistory).map(([key, value]: [string, any]) => ({
              id: key,
              cowId,
              ...value
            }));
