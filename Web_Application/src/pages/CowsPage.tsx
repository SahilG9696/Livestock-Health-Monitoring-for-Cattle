    ...
    const unsubscribe = onValue(cowsRef, (snapshot) => {
      ...
      if (data) {
        const cowsList = Object.keys(data).map(cowId => {
          ...
          const dataArray = Object.values(cowHistory);
          
          if (dataArray.length === 0) return null;

          // Get latest data point
          const latestData: any = dataArray.sort((a: any, b: any) => b.timestamp - a.timestamp)[0];
        ...
        if (selectedCowId && data[selectedCowId]?.history) {
          ...
          const dataArray = Object.values(selectedCowData) as any[];
